...
def process():
    ...
    frame = str(request.args.get('frame'))
    if sleep/waitTime > 5:
        time.sleep(sleep/waitTime)
    else:
        time.sleep(5)
    os.system("scrot output/frame_"+frame+".png")
    # ffmpeg -r 29.97 -i frame_%003d.png test.mkv 
    # -r = the framerate, adjust when needed
    return "frame_" + frame + ".png saved"
app.run()
